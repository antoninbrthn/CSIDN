...
def main():
    ...
    def main_training(mu, iter):
        ...
        if os.path.exists(txtfile):
            ...
            os.system(
                "mv %s %s"
                % (txtfile, save_dir + "/junk/" + model_str + "_bak-%s.txt" % nowTime)
            )

        # Import data
        data = import_data(
            path=args["import_data_path"], dataset_name=dataset_name, val_size=val_size
        )
        ...
        for epoch in range(nb_epoch):
            ...
            train_loss, train_acc = train(
                train_loader, epoch, model, optimizer, criterion
            )
        ...
        plot_contour(
            model, title="Lq - Average noise : {}%".format(str(round(100 * mu, 2)))
        )
