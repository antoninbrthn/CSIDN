...
def main():
    ...
    def main_training(mu, iter, pert_strength):
        ...
        print(
            "RTRAIN MEAN before perturbation: ", data.r_train.mean(), data.r_train.std()
        )
        r_train = pertubate_noise(r_train, pert_strength)
        # r_train = np.ones_like(r_train) * 0.99
        data.r_train = r_train
        print(
            "RTRAIN MEAN after perturbation: ", data.r_train.mean(), data.r_train.std()
        )
        # Get loaders
        ...
        criterion = models_pytorch.loss_general_forward_iterative(
            S, quantiles, mus, args, use="mean", debug=False
        )
