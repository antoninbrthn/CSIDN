...
-----------
Script description:
Main implementation of ILFC for sensitivity analysis on synthetic datasets.
'''
...
# Confidence perturbation strength
pert_strengths = [0.2, 0.4, 0.6]

torch.manual_seed(random_seed)

mus_tab = [0.35, 0.45, 0.5]
...
# Import data
def import_data(path, dataset_name, val_size, dummify = True,
                shuffle = True):
    ...
    for i, (images, labels_clean, labels_noisy, r) in enumerate(train_loader):
        ...
        if epoch >= warm_start:
            ...
            if dummy_labels:
                ...
                noisy_dummies = torch.FloatTensor(args["bs"], args["n_class"]).to(device).zero_().scatter_(1,
                                                                                                           labels_noisy.view(
                                                                                                               -1, 1),
                                                                                                           1).to(device)
...
def main():
    ...
    def main_training(mu, iter, pert_strength):
        print("Beginning noise level {}".format(mu))
        # File to export results during training
        save_dir = args["result_dir"] + args["dataset"] + "_sensitivity" + '/ILFC/{}/'.format(str(iter))
        if not os.path.exists(save_dir):
            os.system('mkdir -p %s' % save_dir)

        dataset_name = args["dataset"] + "_%s-%.2f" % (args["noise_type"], mu)
        model_str = dataset_name + '_noise%.2f' % (pert_strength) + '_ILFC'
        ...
        r_train = pertubate_noise(r_train, pert_strength)
        # r_train = np.ones_like(r_train) * 0.99
        ...
        # Define models
        print("Training naive noisy classifier for {} epochs".format(args["noisy_model_epochs"]))
        ...
        criterion = models_pytorch.loss_general_forward_iterative(S, quantiles, mus, args, use = "mean", debug = False)
        ...
        for epoch in range(nb_epoch):
            ...
            # Train the network
            train_loss, train_acc = train(train_loader, epoch, model, noisy_model, optimizer, criterion)
        ...
        plot_contour(model,
                     title = "ILFC - Average noise : {}%".format(str(round(100 * mu, 2))))
        plt.savefig("figures/synthetic-ILFC-%d.png" % (100 * mu))

    for iter in range(n_iter):
        for mus in mus_tab:
            for pert_strength in pert_strengths:
                main_training(mus, iter, pert_strength)
