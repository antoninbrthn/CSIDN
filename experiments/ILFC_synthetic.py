    ...
    for i, (images, labels_clean, labels_noisy, r) in enumerate(train_loader):
        ...
        if epoch >= warm_start:
            ...
            if dummy_labels:
                ...
                noisy_dummies = torch.FloatTensor(args["bs"], args["n_class"]).to(device).zero_().scatter_(1, labels_noisy.view(-1, 1),
                                                                                      1).to(device)
...
def main():
    ...
    def main_training(mu, iter):
        ...
        # Import data
        data = import_data(path = args["import_data_path"], dataset_name = dataset_name, val_size = val_size, dummify = dummy_labels)
        ...
        criterion = models_pytorch.loss_general_forward_iterative(S, quantiles, args, use="mean", debug=False)
        ...
        print('Epoch [%d/%d] Test Accuracy on the %s test images: Model %.4f %%' % (epoch+1, args["nb_epoch"], len_test, test_acc))
        # save results
        with open(txtfile, "a") as myfile:
