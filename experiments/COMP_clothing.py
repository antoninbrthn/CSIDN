...
parser.add_argument('--model', type = str, help = 'cnn, mlp, large_cnn, resnet18, resnet50', default = 'resnet18')
...
parser.add_argument('--mom_decay_start', type = int, help = 'rate of decrease of the learning rate',
                    default = 40)
parser.add_argument('--num_workers', type = int, default = 8, help = 'how many subprocesses to use for '
                                                                     'data loading')
parser.add_argument('--train_limit', type = int, default = 100000, help = 'max number of batches to '
                                                                          'train on at each epoch')
parser.add_argument('--model_export', type = str, default = '',
                    help = 'dir to export model after training')
parser.add_argument('--model_import', type = str, default = '',
                    help = 'path to import model from')
...
def main():
    ...
    if args["og_dataset"]:
        ...
        validset = Clothing1M(path = data_path, mode = 'clean_val', dataset_type = 'drive',
                              transform = test_transform)
    ...
    for epoch in range(nb_epoch):
        ...
        # Train the network
        train_loss, train_acc = train(train_loader, epoch, model, optimizer,
